...
func loadDataApi(currentPage: Int,Language: String ,onComplete: @escaping (_ repositories:[Items])-> Void) {
    ...
    URLSession.shared.dataTask(with: url) {(data, response, err)in
        ...
        do {
            let course = try JSONDecoder().decode(LanguageItem.self, from: data)
